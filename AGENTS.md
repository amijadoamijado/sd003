...
- Codexは、独立した調査・実装・検証の並列化が有益ならサブエージェントを積極的に活用する。子は原則 `gpt-6-luna`・推論量 `medium` を明示指定し、ユーザーの別途指定を優先する。単純な作業のためだけに増員せず、必要な文脈だけを渡す。編集を並列化するときは担当ファイルを分け、必要ならworktreeで隔離する。既存ブランチの切替、公開、マージは別途その操作の許可範囲に従う。他CLIの並列運用は各Native仕様に従う。
