...
Use this skill when the user asks to run the migrated source command `sessionread`.
...
| 順序 | ファイル | 目的 |
|------|---------|------|
| 1 | `D:\Codex\AGENTS.md` | グローバル設定（UTF-8制約等） |
| 2 | `./AGENTS.md` | プロジェクト設定（SD003ルール） |
| 3 | `.sessions/session-current.md` | 現在のセッション（短期記憶） |
| 4 | `.sessions/TIMELINE.md` | プロジェクト履歴（長期記憶） |

## 実行手順

### Step 1: グローバル AGENTS.md
```
Read: D:\Codex\AGENTS.md
```
- UTF-8境界エラー防止ルール
- 日本語ファイル操作制約
- セッション開始プロトコル

### Step 2: プロジェクト AGENTS.md
```
Read: ./AGENTS.md
```
- SD003フレームワーク設定
...
## 1. グローバル設定
✅ AGENTS.md (グローバル) 読み込み完了
   - UTF-8制約ルール確認

## 2. プロジェクト設定
✅ AGENTS.md (プロジェクト) 読み込み完了
   - SD003 v[バージョン]
...
| ファイル | 存在しない場合 |
|---------|---------------|
| グローバル AGENTS.md | 警告表示、続行 |
| プロジェクト AGENTS.md | 警告表示、続行 |
| session-current.md | 「新規セッション」として扱う |
| TIMELINE.md | 「履歴なし」として扱う |
| SD003アップデートチェック（Step 6） | 作業ディレクトリが `D:\Codex\sd003` 自身、または `./AGENTS.md` に `SD003 v` 表記なし → スキップ（無音） |
...
Agent(run_in_background=true):
  ...
  prompt: |
    古いCodexセッションのアーカイブ状況を確認します。以下を実行してください:
    1. bash ~/.Codex/scripts/archive-sessions.sh 7 preview でプレビュー
...
Step 2（プロジェクト AGENTS.md）読み込み後、非ブロッキングで以下を確認する。
**このプロジェクトが SD003 本体（`D:\Codex\sd003`）自身の場合は完全にスキップする**（無音）。

### 6-1: バージョン検出

1. `Grep pattern:"SD003 v[0-9.]+" path:"./AGENTS.md"` でローカルバージョンを取得。
   マッチしなければ「このプロジェクトはSD003未デプロイ」としてスキップ（無音・以降の手順は実行しない）。
2. `Grep pattern:"FRAMEWORK_VERSION = " path:"D:/Codex/sd003/.Codex/skills/sd-deploy/deploy.ps1"`
   で現行バージョン（`$FRAMEWORK_VERSION`）を取得する。
   - 注意: SD003本体の `AGENTS.md` 末尾表記（例: `SD003 v3.4.0`）とはズレがあり得る
...
1. dry-run:
   ```
   pwsh -File "D:\Codex\sd003\.Codex\skills\sd-upgrade\upgrade.ps1" "<このプロジェクトの絶対パス>"
   ```
   - 必ずSD003本体側の絶対パスのスクリプトを呼ぶこと（`$PSScriptRoot`基準でsourceを解決するため、
     デプロイ先にコピーされた同名スクリプトをそのcwdで実行すると誤動作する）
   - CP932文字化け回避のため `powershell` ではなく `pwsh` を使う（グローバルAGENTS.md既存ルール）
2. dry-run結果（`WILL OVERWRITE - LOCAL CUSTOMIZATION WILL BE LOST` 等）をユーザーに提示する。
   ...
   追記を促す（`.Codex/skills/sd-upgrade/SKILL.md` の `.sd003-keep` 節を参照）。
3. 再度 `AskUserQuestion` で `--execute` で実行してよいか確認する（2段目のゲート。
   破壊的操作のため dry-run結果を見せずに実行しない）。
4. 承認後のみ実行:
   ```
   pwsh -File "D:\Codex\sd003\.Codex\skills\sd-upgrade\upgrade.ps1" "<このプロジェクトの絶対パス>" -Execute
   ```
5. 完了後、`npm install` の実行と（agy利用時は）agy再起動を推奨する旨を案内する
   （`.Codex/skills/sd-upgrade/SKILL.md` の「注意」節に準拠）。
