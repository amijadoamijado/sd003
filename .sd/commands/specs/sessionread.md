...
`.codex/check-framework-version.py`が存在し、このセッションでまだ版を比較していなければ、`python .codex/check-framework-version.py`でローカル更新元と比較する。更新元の指定には`--source <絶対パス>`または`SD003_SOURCE`を使える。更新があれば導入版・更新元版を報告し、参照不能は未確認として扱う。版比較だけではファイルを更新しない。
...
セッションアーカイブ、フレームワークの更新適用、モデル変更は、読み込みのたびに起動しない。上記の版比較とは分け、必要になった時点で対応する。SD003更新を依頼された場合は`sd-upgrade`の手順に従う。
