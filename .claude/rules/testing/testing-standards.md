...
**カバレッジ数値目標は廃止**（`.claude/rules/global/real-data-first.md` 柱3に基づく。2026-06-10にjest coverageThresholdも撤廃済み）。
完了条件は「実データで動作することを実環境で確認」+ VTD-001〜005通過。
カバレッジ数値はテストの良し悪しの指標として使わない。
...
**テストを書く前に、対象のソフトウェアが実環境で動くことを確認していなければならない。**

- 動かないソフトウェアに対するテストは無価値（442個書いても0個と同じ）
- まず手で動かす → 動いたらその動作を保証するテストを書く
- モックの中でモックが正しく動くことの確認はテストではない

### テストを書いてよいタイミング
| 状態 | テストを書くか |
|------|--------------|
| まだ一度も動かしていない | **書かない** |
| 実環境で動作確認済み | **書く**（動作を保証するため） |
| 動いているが不安定 | **書く**（再発防止のため） |
