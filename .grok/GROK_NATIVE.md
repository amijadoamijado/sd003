...
Grokネイティブ予定タスク機能のツールスキーマ: `docs/reference/grok-tasks-mcp/`
...
4. `pwsh -File scripts/lead-lock.ps1 acquire grok` でrepo lockを取得する。
...
6. セッション終了時（または大きな区切り）に生成済み `.grok/skills/sessionwrite` を実行し、session-current.md / TIMELINE.md を更新する。
...
| Quiz Gate 該当（Blueprint級実装の完了主張） | **Codex** | 実装 diff と検証観点を渡して出題を依頼（`quiz-gate.md`。Grok自身がGeneratorのときは自己クイズ禁止） |
