...
"@
Set-Content -Path (Join-Path $TargetProject ".sd\ids\registry.json") -Value $registryContent -Encoding UTF8
...
"@
Set-Content -Path (Join-Path $TargetProject ".sd\ai-coordination\handoff\handoff-log.json") -Value $handoffContent -Encoding UTF8
