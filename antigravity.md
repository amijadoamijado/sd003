...
| Role | Description |
|------|-------------|
| Implementation | Execute direct prompts or formal free-form requests under `spec/{案件ID}/` |
| E2E Test | Execute direct verification prompts or formal free-form requests |
| Report | Create evidence-backed reports under `review/{案件ID}/` |

### Trigger Keywords (AUTO-EXECUTE)

| Keyword | AI | Action |
|---------|-----|--------|
| "implement", "build" | Antigravity | Dispatch the prompt through agy-dispatch; formal requests may live under `spec/{案件ID}/` |
| "test request", "verify" | Antigravity | Execute the direct prompt or a formal free-form request |
...
- ESLint errors = 0
- **.sd/ safe commit**: .sd/変更後は早めにcommit（同一bashが最も安全）。未commitの.sd/変更はwipe時にL4で復元されない。
...
正準経路はアドホックな依頼プロンプト（会話内または `spec/{案件ID}/`）を agy-dispatch へ渡す方式です。
...
未認証・二重起動のまま `agy --sandbox --mode accept-edits --prompt ...` を回さない。逼迫・反復ハング時は
...
### 基本構文
```bash
agy --sandbox --mode accept-edits --prompt "プロンプト"
```

### SD003 Agent Scripts

> `agent-implement.sh` / `agent-pipeline.sh` は廃止導線。新規利用せず agy-dispatch を使う。
...
### Pipeline Flow
```
依頼プロンプト（会話内 or spec/{案件ID}/）または `scripts/orchestrate.js`（2026-07-12実E2E実証済み経路）
    ↓
agent-pipeline.sh:
    ├── Step 1: Antigravity CLI (agy --sandbox --mode accept-edits --prompt ...) → 実装
