...
Codexの実行手順は[CODEX_NATIVE.md](../.codex/CODEX_NATIVE.md)を優先する。以下の画面・URL・DONE.md・判断ごとの確認・全件テスト等の一律手順はCodexへ重ねて適用しない。データ保護、保存先、公開・デプロイの承認条件は共通で維持する。
...
3. 動いたら、変更に必要な検証を行う。新規テストは本番バグの再現・再発防止や実データ検証に必要な範囲で追加する
4. 必要になったら抽象化する
...
承認済みの目的・範囲内の実装判断は、根拠を持って自律的に進める。毎ステップの確認は挟まない。

結果を大きく左右する未確定の要件や、未承認の公開・送信・削除・確定処理がある場合は、必要な選択肢と影響を示して確認する。先に進められる独立した作業を完了し、承認事項は実行直前にまとめる。既に承認された範囲は再確認しない。
...
- [ ] 変更に必要な検証を省略して実装完了とする（新規テストの追加は一律必須にしない。実行できなかった検証は理由とともに報告する）
