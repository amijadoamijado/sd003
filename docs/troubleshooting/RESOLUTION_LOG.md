...
### ac001 も対応済み（2026-07-25 ユーザー承認「消えてもいい」にて）
1. 未コミット99件を scratchpad へバックアップ後にクリーン化
   （`git reset --hard`/`git clean`/`git restore` は sd003 ガードレールが物理ブロック
   → HEAD blob のファイル単位書き戻し + untracked の退避移動 + `git add --renormalize` で等価クリーン化。
   残っていた「消えない M」の正体は autocrlf の EOL フラグ＝内容差ゼロ）
2. `node_modules`（**15,511ファイルが丸ごとtracked**・.gitignore記載なし）を filter-repo で履歴除去
   → リポジトリ 3.6MB、force push 成功（`f100631...f4e38ee`）
3. 文字化け復元39件をバックアップから再適用して commit + push（sessionread.md 配備含む）
4. 追加発見: `kiro-deploy.md` / `kiro-init-project.md` は**移行前から化けていた**（別の過去事故）。
   さらに履歴を遡り 2026-02-08 / 2025-10-19 のクリーン版から復元。
5. 最終スキャン: 275ファイル走査・文字化け残存0件・リモート一致。
