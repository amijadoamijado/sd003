...
SD003フレームワーク（v2.19.1）を新規プロジェクトに展開する。配布版番号の正本はdeploy.ps1 / deploy.shのFRAMEWORK_VERSION。deployツールの版番号とは区別する。

`.sd003-keep`でフォルダ全体を保護している場合は、そのフォルダを更新前バックアップへ再コピーしない。保護対象の原本を維持し、更新対象のバックアップは従来どおり作成する。
