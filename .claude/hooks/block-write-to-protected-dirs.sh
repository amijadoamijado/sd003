...
#
# ブロック: .kiro/ .claude/ .handoff/ 内での uv init, pip install, python -m venv 等
# ブロック: .kiro/ .claude/ .handoff/ 内での mkdir ...env/venv/test 等
...
# 保護対象ディレクトリのパターン
PROTECTED_PATTERN='(\.kiro|\.claude|\.handoff)/'
    ...
    cat <<'EOF'
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "BLOCKED: .kiro/ .claude/ .handoff/ 内でのvenv/パッケージ環境の作成は禁止です。テスト環境は D:/claudecode/cache/ 等の隔離ディレクトリに作成してください。理由: browser-use検証時に.kiro/内にvenvを作成し、クリーンアップで.kiro/全体が消失した事故(2026-03-21)の再発防止。"
  }
}
EOF
    exit 0
    ...
    cat <<'EOF'
{
  "hookSpecificOutput": {
    "hookEventName": "PreToolUse",
    "permissionDecision": "deny",
    "permissionDecisionReason": "BLOCKED: .kiro/ .claude/ .handoff/ 内でのPythonプロジェクトファイル作成は禁止です。テスト環境は D:/claudecode/cache/ に作成してください。"
  }
}
EOF
    exit 0
